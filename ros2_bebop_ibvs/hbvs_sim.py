...
class Controller(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('gain_v', 1.)
        ...
        self.kv = self.get_parameter('gain_v').value
    ...
    def control(self, _image):
        ...
        self._u[:3] = self.kv * self.error[:3]
        self._u[3:] = self.kw * self.error[3:]
        return _image
    ...
    def control_loop(self):
        ...
        if not self.cv_image is None:
            ...
            cv2.aruco.drawDetectedMarkers(_image, self.view_corners, self.view_ids,
                                            borderColor = (0, 100, 0.) )
            cv2.aruco.drawDetectedMarkers(_image, self.corners_ref,
                                          self.ids_ref,
                                            borderColor = (0, 0., 100.) )
        ...
        if self.state == IDLE:
            ...
            #   Change state
            if self.new_state == HBVS and self.takeoff_complete:
                self.get_logger().info("State change: HBVS")
                self.state = HBVS
            elif self.new_state == HBVS and  not self.takeoff_complete:
                self.get_logger().info("Waiting for TAKEOFF to finish, can not change to HBVS")
            ...
            #   Change state
            if self.new_state == HBVS and self.init_complete:
                self.get_logger().info("State change: HBVS")
                self.state = HBVS
            elif self.new_state == HBVS and  not self.init_complete:
                self.get_logger().info("Waiting for INITIAL CONDITION to finish, can not change to HBVS")
            elif self.new_state == LANDING:
                self.get_logger().info("State change: LANDING")
                self.state = LANDING
            elif self.new_state == STOP:
        ...
        elif self.state == HBVS and self.points is None:
            ...
            try:
                ...
                self.get_logger().error(f"Error with HBVS control: {str(e)}")
                self.enable = False
                self.cmd_enable.publish(Bool(data=self.enable))
            #   Change state
